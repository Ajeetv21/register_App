  ...
  return (
   ...
   <ol className='flex justify-start flex-col mt-5 w-1/2 m-auto container'>
    ...
    ?users
          .map((user,index)=>{
            return(
              <li key={index} className='flex justify-between mb-4 text-1xl p-4 bg-indigo-100'>
                <Link className='hover:text-blue-400 ' to={`/show/details/${user.username }`} style={mycss}>{user.username}</Link>
                <p>
